...
test(function throwsOnInvalidDigestLength(): void {
  try {
    new Blake2b(0);
    assert(false);
  } catch (err) {
...
test(function passesTestVectors(): void {
  testVectors.map(parseTestData).forEach(
    async ({ outlen, input, key, salt, personal, expected }): Promise<void> => {
...
test(async function allowsMultipleWrites(): Promise<void> {
...
test(async function allowsUnsafeShortDigest(): Promise<void> {
...
test(async function allowsMacinWithKeyAndShortDigest(): Promise<void> {
  const key: Uint8Array = new Uint8Array(32).fill(108);
  for (let i = 1; i < key.length; i += 2) {
    key[i] = 111;
  }
